...
This file creates a word embedding model using all answers by applicants, then creates a sentence embedding finding the centroid
embedding along each word in the sentence and then cosine similarity between each centroid sentence to find their "linguistic similarity score"

Run Command: python AnswerCorpus_Centroid.py
...
def createEmbeddingSpace(txt_Q1, txt_Q2, txt_Q3):
    ...
    sentQ1 = [row.split(" ") for row in txt_Q1 if row != None]
    sentQ2 = [row.split(" ") for row in txt_Q2 if row != None]
    sentQ3 = [row.split(" ") for row in txt_Q3 if row != None]

    sentencesAll = sentQ1 + sentQ2 + sentQ3
...
#Find Average point for each sentence within the embedding space
def vocabFilter(sentence):
    return [word for word in sentence if (word in w2v_model.wv.vocab)]

def findAverage(sentence):
    return np.mean(w2v_model.wv[vocabFilter(sentence)], axis = 0)

def centroid(sentence_pair):

    similarity = 0

    for pair in sentence_pair:

        sent_1_score = findAverage(pair[0])
        sent_2_score = findAverage(pair[1])

        #score += np.dot(sent_1_score, sent_2_score) / (np.linalg.norm(sent_1_score) * np.linalg.norm(sent_2_score))
        similarity += (1 - spatial.distance.cosine(sent_1_score, sent_2_score))
...
def findSimilarityScore(sent_Q, hired_Q):
    ...
    all_similarity_score = pool.map(func=centroid, iterable=all_sentence_pairs)
...
if __name__ == '__main__':
    ...
    w2v_model = createEmbeddingSpace(txt_Q1, txt_Q2, txt_Q3)
    ...
    np.savetxt('np_q1_keep_Q1_index.txt', np.array(keep_index_Q1))
    ...
    np.savetxt('np_q1_similarity_score_centroid.txt', np_q1_similarity_score)
    ...
    np.savetxt('np_q2_keep_Q2_index.txt', np.array(keep_index_Q2))
    ...
    np.savetxt('np_q2_similarity_score_centroid.txt', np_q2_similarity_score)
    ...
    np.savetxt('np_q3_keep_Q3_index.txt', np.array(keep_index_Q3))
    ...
    np.savetxt('np_q3_similarity_score_centroid.txt', np_q3_similarity_score)
