...
This file uses pre-trained Universal Sentence Encoder embeddings on applicant sentences, and uses paired cosine similairty to
to find their "linguistic similarity score"

Run Command: python USE_Google.py
...
def findUSESimilarityScore(sent_Q, hired_Q):
    ...
    all_similarity_score = pool.map(func=USEscore, iterable= all_sentence_pairs)
...
if __name__ == '__main__':
    ...
    q1_USE_similarity_score = findUSESimilarityScore(sent_Q1, hired_Q1)
    print("...Q1 USE Similarity Score was found...\n")
    np_q1_USE_similarity_score = np.array(q1_USE_similarity_score)
    np.savetxt('np_q1_USE_similarity_score.txt', np_q1_USE_similarity_score)
    ...
    hired_Q2_index = list(compress(range(len(sent_Q2_index)), sent_Q2_index)) #index of hired application from Q2
    hired_Q2 = list(compress(sent_Q2, sent_Q2_index)) #sentence only from hired applicants

    print("...Q2 Analysis ready to start...\n")
    q2_USE_similarity_score = findUSESimilarityScore(sent_Q2, hired_Q2)
    print("...Q2 USE Similarity Score was found...\n")
    np_q2_USE_similarity_score = np.array(q2_USE_similarity_score)
    np.savetxt('np_q2_USE_similarity_score.txt', np_q2_USE_similarity_score)
    ...
    hired_Q3_index = list(compress(range(len(sent_Q3_index)), sent_Q3_index)) #index of hired application from Q3
    hired_Q3 = list(compress(sent_Q3, sent_Q3_index)) #sentence only from hired applicants

    print("...Q3 Analysis ready to start...\n")
    q3_USE_similarity_score = findUSESimilarityScore(sent_Q3, hired_Q3)
    print("...Q3 USE Similarity Score was found...\n")
    np_q3_USE_similarity_score = np.array(q3_USE_similarity_score)
    np.savetxt('np_q3_USE_similarity_score.txt', np_q3_USE_similarity_score)
